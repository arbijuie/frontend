  ...
  return (
    ...
    >
      ...
      <div className={styles.grid}>
        ...
        <span className={styles.rowValue}>{asNumber(preset.values.min_score_bps)} bps</span>
        <span className={styles.rowLabel}>Min Volume 24h</span>
        <span className={styles.rowValue}>
          ${asNumber(preset.values.min_volume_24h).toLocaleString()}
        </span>
        <span className={styles.rowLabel}>Min Open Interest</span>
        <span className={styles.rowValue}>
          ${asNumber(preset.values.min_open_interest).toLocaleString()}
        </span>
        <span className={styles.rowLabel}>Min Persistence</span>
        <span className={styles.rowValue}>{asNumber(preset.values.min_persistence_hours)}h</span>
        <span className={styles.rowLabel}>Anti-Churn Cooldown</span>
        <span className={styles.rowValue}>{asNumber(preset.values.anti_churn_cooldown_s)}s</span>
      </div>
    </button>
  );
};
